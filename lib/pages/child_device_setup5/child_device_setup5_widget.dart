...
class _ChildDeviceSetup5WidgetState extends State<ChildDeviceSetup5Widget> {
  ...
  Future<void> _startChildMode() async {
    try {
      ...
      // Enforce single-device-per-profile rule
      final claimed = await DeviceStatusService.claimDevice(deviceId);
      if (!claimed) {
        if (mounted) {
          await showDialog(
            context: context,
            builder: (ctx) => AlertDialog(
              ...
              content: const Text(
                'This child profile is already active on another device. '
                'Only one device is allowed per profile at a time.\n\n'
                'Please deactivate child mode on the other device first.',
              ),
              actions: [
                ElevatedButton(
